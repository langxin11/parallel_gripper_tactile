...
class CommandConfig(_TaskModel):
    ...
    max_dynamic_step: Annotated[int, Field(gt=0)] = 3
...
class DeadbandConfig(_TaskModel):
    ...
    tick_factor_alpha: Annotated[FiniteFloat, Field(ge=0)] = 0.6
...
def run_robotiq_discrete_force(
    ...
    *,
    ...
    """运行一次离散力控制仿真并保存按控制周期采样的压缩轨迹和诊断图。"""
    ...
    for step_index in range(steps):
        ...
        # trace 按控制周期采样；动作生效行（含延迟写入）强制保留，避免丢失动作时刻。
        trace_keep.append(step_index == 0 or applied_delta != 0.0 or control_dt_s is not None)
        rows.append(
            {
    ...
    trace_keep[-1] = True
    trace_rows = [row for row, keep in zip(rows, trace_keep) if keep]
    _write_trace(trace_rows, output_csv)
    _plot_trace(rows, output_plot)
    forces = np.asarray([row["force_filtered_n"] for row in rows], dtype=float)
    commands = np.asarray([row["u"] for row in rows], dtype=float)
    states = np.asarray([row["state"] for row in rows], dtype=str)
    contact = forces >= effective_control.contact_threshold_n
    tracking = np.asarray([row["tracking_time_s"] != "" for row in rows], dtype=bool)
    evaluation = tracking if np.any(tracking) else np.ones_like(tracking)
    targets = np.asarray([row["force_target_n"] for row in rows], dtype=float)
    errors = forces[evaluation] - targets[evaluation]
    tracking_times = np.asarray(
        [float(row["tracking_time_s"]) for row in rows if row["tracking_time_s"] != ""],
        dtype=float,
    )
    ...
    if discrete is not None:
        ...
        delta_samples = 0
        prediction_mae = None
    finite = bool(np.all(np.isfinite(forces)) and np.all(np.isfinite(commands)))
    safety_violated = bool(np.max(forces) > effective_control.max_force_n)
    integer_commands = bool(np.allclose(commands, np.rint(commands), atol=1e-12))
    ...
    final_mask = (
        ...
        & (
            np.asarray(
                [row["tracking_time_s"] if row["tracking_time_s"] != "" else -1 for row in rows],
                dtype=float,
            )
