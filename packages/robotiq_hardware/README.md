...
可选依赖精确固定为 `pyrobotiqgripper==3.3.12`。版本化文档：[pyrobotiqgripper 3.3.12 API](https://pypi.org/project/pyrobotiqgripper/3.3.12/)。`PyRobotiqGripper3312Backend` 只使用该版本已确认的 `move` 调用形态：
...
该后端尚未经过真实 Robotiq 设备验证。连接、激活、串口参数和设备生命周期全部由调用方负责；测试只使用 fake gripper，不会发送真实设备命令。
