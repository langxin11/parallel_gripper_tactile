...
- `dmgripper-run` 的使能前零力验证改为只以双侧滤波 `Fz` 窗口均值为门禁；门禁通过后，验证期内任一
  峰值达到 `lifecycle.contact_on_n` 时只记录一次含最大峰值、触发侧和阈值的结构化 `warning`，不再重置
  窗口或单独阻止进入 `ready`。原始法向力上限、双侧不平衡及其他运行期安全保护不变。
...
- `preload` 稳定判据新增独立高侧容差 `preload_overforce_tolerance_n`，默认仍与原对称容差一致；
  `adaptive_grip.yaml` 只把高侧放宽到 `0.3 N`，接纳实测温和过冲而不降低低侧抓力要求，并保留导纳
  单向闭合。预载超时信息同时新增目标、当前双侧力、均值、允许区间及 `prevent_unloading` 阻塞线索。
