...
def test_adaptive_multirate_preserves_transient_admittance_and_filter_overrides(tmp_path) -> None:
    ...
    result = run_force_scheduling(
        candidate.profile,
        task=candidate.task,
        scheduler_config=candidate.scheduler,
        output_csv=trace,
    )
...
def test_multirate_rejects_sampling_period_not_aligned_to_physics() -> None:
    ...
    with pytest.raises(ValueError, match="整数倍"):
        run_force_scheduling(resolved.profile, task=misaligned, scheduler_config=resolved.scheduler)
...
def test_gravity_hold_uses_small_target_without_slip() -> None:
    ...
    result = run_force_scheduling(PROFILE, task=task, scheduler_config=ORACLE)
...
def test_dynamic_filling_increases_target_and_remains_stable(tmp_path: Path) -> None:
    ...
    result = run_force_scheduling(
        PROFILE, task=task, scheduler_config=ORACLE, output_csv=trace_path
    )
...
def test_noslip_solver_does_not_hide_insufficient_grip(mode: str) -> None:
    ...
    if mode == "adaptive":
        ...
        scheduler_config = replace(
            resolved.scheduler, load=replace(resolved.scheduler.load, max_force_n=0.5)
        )

    result = run_force_scheduling(profile, task=weak_task, scheduler_config=scheduler_config)
...
@pytest.mark.parametrize("mode", ["oracle", "adaptive", "multirate"])
def test_execute_force_scheduling_writes_reproducible_artifacts(
    ...
    run, result = execute_force_scheduling(
        profile=PROFILE,
        resolved_profile=profile if mode in {"adaptive", "multirate"} else None,
    ...
    assert effective["runtime"]["scheduler_kind"] == ("adaptive" if mode == "multirate" else mode)
    if mode in {"adaptive", "multirate"}:
        assert metrics["failure_reason"] is None
        assert effective["scheduler"]["risk_enabled"] is False
    if mode == "multirate":
        samples = [
...
def test_adaptive_simulation_tracks_measured_load(tmp_path: Path, scenario: str) -> None:
    ...
    max_rate = scheduler_config.load.max_force_rate_n_s
    ...
    if scenario == "step":
        assert targets[0] == 1
        assert scheduler_config.load.max_force_rate_n_s == 50
