...
def test_default_study_expands_complete_ablation_matrix() -> None:
    ...
    config = load_robotiq_discrete_force_study_config(
        ROOT / "configs/research/robotiq_discrete_force_validation/study.yaml"
    )
    ...
    assert config.task == (ROOT / "configs/task/discrete_force/robotiq_delta_f_tick.yaml").resolve()
