...
指标覆盖安全超限、动作与反转、HOLD、稳定时间、力误差和预测误差，并提供七个平台的独立统计。
稳态 MAE 取逐点绝对误差，不能以正负抵消。动作统计按实际整数命令变化计算。
`passed` 要求仿真有限、未超过安全力、建立接触、所有平台均有至少 0.5 s 持续 HOLD，且命令全为整数。
