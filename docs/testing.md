...
- 摩擦估计保留完整“仿真—轨迹处理—双格式绘图—manifest”端到端测试，低、中、高摩擦和双倍噪声
  四个标准场景及保守性、保持稳定性断言全部保留。
- 绘图内容测试使用小型合成数据，验证布局选择、统计值、缺失数据处理、PNG／PDF 工件登记和非空
  输出；测试内以低分辨率 PNG 完成真实内容渲染，PDF 分支使用最小画布，避免同一内容重复执行昂贵布局。
- `tests/test_plotstyle.py` 集中真实验证 IEEE 栏宽、600 DPI PNG、PDF MediaBox、字体嵌入配置和中文
