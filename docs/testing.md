...
- 标准摩擦场景和旧参数兼容性对照在同一测试进程内复用名义场景仿真；fixture 返回深拷贝，
  防止嵌套诊断字段互相污染。四个标准物理场景、修改参数后的对照和 runner 端到端仍全部执行；
  xdist worker 各自缓存，不能据此保证并行全量同等提速。
- 静态迁移快照的哈希与历史锚点合并检查；它只验证历史文件完整性，当前行为仍由配置组合、
  有序矩阵和实际运行测试验证。共同的计划产物断言使用参数化，保留每项研究的独立场景。
- 配置发现、进度展示和生命周期的增量映射包含新增行为测试；CI 另在安装开发依赖之前检查
  最小运行环境中的 CLI，避免开发依赖掩盖运行依赖缺失。
