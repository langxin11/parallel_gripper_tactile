...
这说明当前仿真的逐 taxel 接触筛选和比值场只完成了局部估计的力观测部分。现有触觉接口没有独立的
taxel 切向位移或速度，因而不能复现论文中的局部滑移判据，也不能把局部比值峰值解释为摩擦系数。
下一阶段应先增加与硬件可观测量一致的局部切向运动代理，再按“参考 taxel → 相对速度持续下降 →
锁存事件前比值”的顺序实现两侧独立估计。论文使用最新事件直接覆盖旧估计；本项目还需对比该策略与
保守分位数/下界锁存策略，避免单个噪声事件使目标力不足。
