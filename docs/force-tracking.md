...
`pid-torque-ff` 的位置式 PID 同样把实际 \(T_c\) 用于积分和微分，因此其增益不是按“每周期”定义，
可视为基本控制频率解耦；但不能表述为完全不受控制频率影响。采样保持、离散相位、MIT 命令量化、
接触确认步数和接触动力学仍会改变闭环响应。在 stiff Step 三 seed 验证中，其平均 RMSE 在
500／250／125 Hz 下分别为 0.382／0.396／0.432 N，平台力标准差分别为
0.0078／0.0107／0.0171 N。频率降低后性能缓慢退化，但没有出现刚度位置限幅的极限环。
