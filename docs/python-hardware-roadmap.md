...
子域整理与 Robotiq 离散控制核心提取，DM 力控算法族（MIT 协议量化、在线刚度估计、
二阶直接力矩 LADRC 与法向力外环四条路径）已迁入核心，仿真侧 `control.py` 改为适配层；
P3 已完成 DM USB2CAN 状态刷新／MIT 离线适配、Robotiq 命令／反馈／离散控制单步，以及
PapillArray PTS v2.0 解析／同步串口边界，并已完成 DM 与 PapillArray 的有限次数真机探针；
P4 已完成首次受限 DM 真机动作观察，异步运行时、正式记录和完整基础闭环仍待接入。
...
| P6：完整算法迁移（部分完成） | DM 力控算法族已核心化，仿真侧收敛为适配层；剩余：真机接入后的参数验证、Robotiq 自适应／预测复验与统一分析入口 | 按夹爪同条件比较，区分仿真与实机结论，完成回放和文档同步 |
