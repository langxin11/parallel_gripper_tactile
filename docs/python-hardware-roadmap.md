...
离散控制核心提取；P3 已建立两种夹爪的无设备 I/O 硬件基础包，真实传输与触觉采集仍待接入。
...
| P3：纯 Python 采集（进行中） | 已建立 DM 协议／分帧／fake transport 与 Robotiq 3.3.12 非阻塞适配；待触觉读取、真实传输、日志及设备状态 | fake 串口／回放覆盖拆帧、错误帧、重复帧、超时、断连、旧反馈；待设备在线后验证真实采集 |
