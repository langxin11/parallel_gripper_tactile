...
共享场景默认物理步长为 1 ms（1000 Hz），仿真多速率触觉默认采样周期同为 1 ms。
随后控制时钟读取最新快照并更新目标／MIT 请求。1000 Hz 采样、250 Hz 控制时，控制序号正常跨四个采样；
...
DM 的可选 `dmgripper-teleop` 入口独立拥有摇杆速度映射、角目标积分、回中保持和
输入失效失能策略，单个设备线程复用 `DmSession`，不依赖触觉或实验包；这些策略不下沉到会话类。
硬件对象构造不发生 I/O；连接、使能与动作必须显式调用。DM 真机实验入口与故障处理见
